...
AFTER INSERT ON juja0400.product
FOR EACH ROW
...
-- Rule 14: Check if there is enough stock for the order
...
-- Rule 17: Check if product name is unique within the same category
CREATE OR REPLACE FUNCTION check_unique_product_name()
RETURNS TRIGGER AS $$
BEGIN
    IF EXISTS (
        SELECT 1 
...
-- Rule 18: Check if email format is valid
...
-- Rule 21: Validate warehouse assignment for employee
CREATE OR REPLACE FUNCTION validate_warehouse_assignment()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
...
CREATE OR REPLACE FUNCTION prevent_inactive_product_orders()
RETURNS TRIGGER AS $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
